...
本能力**只到提示詞資料維護為止**：提示詞從未被送進任何模型。

> 0.4.27 更新：系統已會呼叫 Azure OpenAI 進行**語音轉錄**（見 [會議紀錄 PRD](會議紀錄-prd.md)），`LlmSettings` 不再只是骨架。但**文字生成端仍無呼叫端**——「把提示詞代入逐字稿產生會議紀錄草稿」尚未實作，見 [會議紀錄產生流程 PRD](會議紀錄產生流程-prd.md)。本節其餘內容描述 0.4.26 的實作現況，未受 0.4.27 影響。
